...
export default function MovesLogicMachine({ pokemon }) {
    const typeImages = {
    ...
    const generations = Object.keys(pokemon.moves)
        ...
        .filter(gen => pokemon.moves[gen].some(move => move.learn_method === "machine"));
        ...
        return (
            <section className="moves-logic-container">
                <section className="moves-logic-header">
                    <h2>moves learnt by machine</h2>
                </section>
                <p style={{ textAlign: "center", margin: "20px", fontFamily: "Sour Gummy" }}>No machine moves found for this form.</p>
            </section>
        );
    }
                        ...
                        return (
                            <section onMouseLeave={hideDetails} className="move-row" key={`${currentGenName}-${move.name}-${index}`}>
                                <section className="move-and-level">
                                    <h4 onClick={() => showDetails(move.name)} style={{ marginLeft: "20px" }}>{move.name}</h4>
                                </section>
